...
describe('detectStableIncome', () => {
  const salaryMonths = [
    stmt('2026-01', 1, [
      tx('s1', '2026-01-28', 'Virement ACME Corp salaire janvier', 2800),
    ]),
    stmt('2026-02', 2, [
      tx('s2', '2026-02-27', 'Virement ACME CORP Salaire fevrier', 2950),
    ]),
    stmt('2026-03', 3, [
      tx('s3', '2026-03-29', 'ACME Corp Salary', 2750),
  ...
  it('montants instables (>±25%) → cluster rejeté', () => {
    ...
    expect(detectStableIncome(wobbly, new Set(), null).source).toBe(
      'unavailable',
    );
  });
  ...
  it("changement d'employeur : ancien cluster absent du dernier mois + nouveau gros crédit → transition", () => {
    const months = [
      ...salaryMonths,
      stmt('2026-04', 4, [
        tx('n1', '2026-04-28', 'Virement NewJob SAS salaire', 3200),
      ]),
    ];
  ...
  it('revenu fantôme (F5) : cluster stable 3 mois puis absent du dernier mois SANS aucun successeur → unavailable, pas la médiane historique', () => {
    const months = [
      ...salaryMonths,
      stmt('2026-04', 4, []), // dernier mois : aucune transaction du tout
    ];
    const out = detectStableIncome(months, new Set(), null);
    expect(out.source).toBe('unavailable');
    expect(out.monthly).toBeNull();
  ...
  it('excludedTxIds (F7, ex-drawTxIds) : sémantique élargie — un virement entrant récurrent 3 mois dont les txIds sont exclus (ex : épargne entrante) → pas compté comme revenu', () => {
    const savingsMonths = [
      stmt('2026-01', 1, [
        tx('e1', '2026-01-10', 'Virement vers Livret A', 500),
      ]),
      stmt('2026-02', 2, [
        tx('e2', '2026-02-10', 'Virement vers Livret A', 500),
      ]),
      stmt('2026-03', 3, [
        tx('e3', '2026-03-10', 'Virement vers Livret A', 500),
      ]),
    ];
    const out = detectStableIncome(
      savingsMonths,
      new Set(['e1', 'e2', 'e3']),
      null,
    );
  ...
  it('les tirages ne forment jamais un cluster de revenu', () => {
    const drawsOnly = [
      stmt('2026-01', 1, [
        tx('d1', '2026-01-10', 'Virement CrediCorp reserve', 650),
      ]),
      stmt('2026-02', 2, [
        tx('d2', '2026-02-10', 'Virement CrediCorp reserve', 650),
      ]),
      stmt('2026-03', 3, [
        tx('d3', '2026-03-10', 'Virement CrediCorp reserve', 650),
      ]),
    ];
    const out = detectStableIncome(
      drawsOnly,
  ...
  it('médiane sur 3 derniers MOIS calendaires distincts (pas occurrences)', () => {
    ...
    const months = [
      stmt('2026-01', 1, [
        tx('s1', '2026-01-15', 'Virement ACME salary', 1000),
      ]),
      stmt('2026-02', 2, [
        tx('s2', '2026-02-15', 'Virement ACME salary', 1000),
      ]),
      stmt('2026-03', 3, [
        tx('s3', '2026-03-15', 'Virement ACME salary', 1000),
      ]),
      stmt('2026-04', 4, [
        tx('s4a', '2026-04-05', 'Virement ACME salary', 1000),
  ...
  it('transition multi-cluster : somme médianes tous clusters', () => {
    const months = [
      stmt('2026-01', 1, [
        tx('s1', '2026-01-15', 'Virement ACME salary', 2800),
        tx('b1', '2026-01-10', 'Bonus Consulting', 400),
      ]),
      stmt('2026-02', 2, [
        tx('s2', '2026-02-15', 'Virement ACME salary', 2800),
        tx('b2', '2026-02-10', 'Bonus Consulting', 400),
      ]),
      stmt('2026-03', 3, [
        tx('s3', '2026-03-15', 'Virement ACME salary', 2800),
        tx('b3', '2026-03-10', 'Bonus Consulting', 400),
      ]),
      stmt('2026-04', 4, [
        tx('s4', '2026-04-10', 'Bonus Consulting', 400),
