...
import { CandidateCluster, ClusterClassification } from '../../models/credit-detection.model';
...
function makeCluster(key: string): CandidateCluster {
  return {
    ...
    merchant: null,
    occurrences: [
      { date: '2026-01-10', amount: -50, description: `${key} 1`, transactionId: `${key}-a`, statementId: '2026-01' },
      { date: '2026-02-10', amount: -50, description: `${key} 2`, transactionId: `${key}-b`, statementId: '2026-02' },
    ],
  };
}

function makeClassification(overrides: Partial<ClusterClassification> = {}): ClusterClassification {
  return {
...
const stmt = (id: string): MonthlyStatement => ({
  id, month: 1, year: 2026, uploadedAt: '', bankName: 'X', accountHolder: 'Demo', currency: 'EUR',
...
describe('CreditDetectionService', () => {
  let clustering: jest.Mocked<Pick<CandidateClusteringService, 'buildClusters'>>;
  ...
  beforeEach(() => {
    ...
    storage = { getAllStatements: jest.fn().mockResolvedValue([stmt('2026-01'), stmt('2026-02')]) };
  ...
  it('a) scanAll : 2 clusters -> classify 2x, validate 2x, {2, N, []}', async () => {
    ...
    expect(result).toEqual({ clustersAnalyzed: 2, suggestionsCreated: 2, errors: [] });
  });

  it('b) scanAll : 1 cluster en erreur JSON -> errors[1], l\'autre traité', async () => {
    const clusters = [makeCluster('a'), makeCluster('b')];
    clustering.buildClusters.mockReturnValue(clusters);
    classifier.classify.mockImplementation(async (cluster: CandidateCluster) => {
      if (cluster.key === 'a') throw new Error('Réponse Ollama invalide: JSON illisible');
    ...
    expect(result.errors).toEqual([{ clusterKey: 'a', message: 'Réponse Ollama invalide: JSON illisible' }]);
  ...
  it('d) scanStatement : ne cluster que le statement passé', async () => {
    ...
    expect(clustering.buildClusters).toHaveBeenCalledWith([single], expect.any(Set));
  });

  it('e) confidence basse -> suggestionsCreated 0 sans erreur', async () => {
    const clusters = [makeCluster('a')];
    clustering.buildClusters.mockReturnValue(clusters);
    classifier.classify.mockResolvedValue(makeClassification({ confidence: 0.2 }));
    validator.validate.mockResolvedValue({ created: false, reason: 'low_confidence' });
